...
def run(file_list):
    ...
    prediction_list = []
    model = load_model('/kaggle/input/trained-model/candidate.h5', custom_objects=custom_objs)
    ...
    for i in range(len_file_list):
        ...
        try:
            ...
            if len(faces) > 0:
                ...
                for face in faces:
                    face_len = len(face)
                    pad_len = SEQ_LEN - face_len
    #                 faces = 2 * np.random.random_sample((SEQ_LEN, TRAIN_FACE_SIZE, TRAIN_FACE_SIZE, 3)) - 1.0
                    face = np.array(face, dtype=np.float32) / 255.0
                    mask = np.ones(face_len, dtype=np.float32)
                    if pad_len > 0:
                        face = np.pad(face, ((0, pad_len), (0, 0), (0, 0), (0, 0)))
                        mask = np.pad(mask, (0, pad_len))
                    prediction_faces.append(face)
                    masks.append(mask) 
    #                 print(masks)
                prediction_faces = np.array(prediction_faces)
                masks = np.array(masks)
                model_prediction = model.predict([prediction_faces, masks]).flatten()
