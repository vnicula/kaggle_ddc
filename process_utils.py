        ...
        if success:
            ...
            if i % (skip_n+1) == 0:
                ...
                if success:
                    ...
                    if img_scale < 1.0:
                        imr = cv2.resize(
                            im, (int(im.shape[1] * img_scale), int(im.shape[0] * img_scale)))
                    else:
                        imr = im
                    imgs.append(im)
                    imrs.append(imr)
    ...
    for track in tracks:
        ...
        for i, bbox in enumerate(track['bboxes']):
            ...
            if frame_indx < len(originals):
                ...
                face_extract = original[y:y+h, x:x+w].copy()
...
def detect_faces_bbox(detector, label, originals, images, batch_size, img_scale, face_size, keep_tracks):
    ...
    for lb in np.arange(0, len(images), batch_size):
        imgs_pil = [Image.fromarray(image)
                    for image in images[lb:lb+batch_size]]
        frames_boxes, frames_confidences = detector.detect(
            imgs_pil, landmarks=False)
    ...
    tracks = iou_tracker.track_iou(
        detections, 0.8, 0.9, 0.1, constants.MIN_TRACK_FACES)
    ...
    if face_size > 0:
        for track_face in faces:
            for i in range(len(track_face)):
                track_face[i] = cv2.resize(
                    track_face[i], (face_size, face_size), interpolation=cv2.INTER_LINEAR)
    # print(faces, tracks)
    return faces, tracks[:keep_tracks]
