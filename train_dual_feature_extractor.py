...
class TbAugmentation:
    ...
    def __call__(self, image, label):
        augmented_image, _ = augmented_image.image_augment2(image, label)
...
def prepare_dataset(ds, is_training, batch_size, cache):
    ...
    if is_training:
        ...
        ds = ds.map(augment_image.image_augment2, num_parallel_calls=tf.data.experimental.AUTOTUNE)
    # else:
...
def compile_model(model, mode, lr):
    ...
    if mode == 'train' or mode == 'tune':
        if CMDLINE_ARGUMENTS.model_name == 'vggface' or CMDLINE_ARGUMENTS.model_name == 'facenet':
            ...
            # )
            pass
        elif 'meso' in CMDLINE_ARGUMENTS.model_name or CMDLINE_ARGUMENTS.model_name == 'resnet':
            pass
        else:
