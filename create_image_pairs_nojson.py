...
def run56(fake_dir, real_dir):
    ...
    for path_name in tqdm.tqdm(fake_file_list):
        ...
        fake_name = file_name.split('__')[1]
        source_no = file_name.split('_')[0]
        target_no = file_name.split('_')[1].split('.')[0]
        video_no = file_name.split('.')[1]
        real_path_source = os.path.join(reading_dir_0, source_no + '__' + fake_name + '.' + video_no + '.png')
        real_path_target = os.path.join(reading_dir_0, target_no + '__' + fake_name + '.' + video_no + '.png')
        
        print(real_path_source, real_path_target)
        # assert os.path.exists(real_path_source) and os.path.exists(real_path_target)
        f_name = source_no + '_' + target_no + '__' + fake_name + '_' + video_no
        ...
        if os.path.exists(real_path_source):
            ...
            if label1 == 0:
                sample_name = f_name + '.' + source_no + '.png'
                ...
                sample_name = source_no + '.' + f_name + '.png'
...
def run54(fake_dir, real_dir):
    ...
    for path_name in tqdm.tqdm(fake_file_list):
        ...
        # print(path_name, real_path_source)
        # assert os.path.exists(real_path_source) and os.path.exists(real_path_target)
        ...
        if os.path.exists(real_path_source):
            ...
            if label1 == 0:
                sample_name = source_no + '_fake.' + video_no + '.' + source_no + '.png'
                sample_file = os.path.join(writing_dir_0, sample_name)
                imageio.imwrite(sample_file, np.concatenate([fake_img, real_img_source], axis=1))
            else:
                sample_name = source_no + '.' + source_no + '_fake.' + video_no + '.png'
                sample_file = os.path.join(writing_dir_1, sample_name)
                imageio.imwrite(sample_file, np.concatenate([real_img_source, fake_img], axis=1))

        if not os.path.exists(real_path_source):
            print('Could not find a match for %s, source real %s does not exist' %(path_name, real_path_source))
...
if __name__ == '__main__':
    ...
    if args.mode == 0:
        ...
        run54(args.fake_dir, args.real_dir)
    elif args.mode == 56:
        run56(args.fake_dir, args.real_dir)
